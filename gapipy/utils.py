...
# simple cache to avoid computing the {resource_name -> resource} map
# every time get_resource_class_from_resource_name is called
__resource_name_mapping = None

def get_resource_class_from_resource_name(name):
    if __resource_name_mapping is None:
        __resource_name_mapping = {
            resource._resource_name: resource
            for resource in get_available_resource_classes()
        }
    # this will raise KeyError for invalid name
    return __resource_name_mapping[name]
